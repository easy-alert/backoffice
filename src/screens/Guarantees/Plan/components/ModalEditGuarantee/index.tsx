...
// GLOBAL TYPES
import type { IUpdateGuarantee } from '@services/apis/updateGuarantee';
import type { IGuarantee } from '@customTypes/IGuarantee';

// STYLES
import * as Style from './styles';

interface IModalEditGuarantee {
  guarantee: IGuarantee;
  handleEditGuaranteePlan: (values: IUpdateGuarantee) => Promise<void>;
  handleModalEditGuarantee: (state: boolean) => void;
}
...
export const ModalEditGuarantee = ({
  guarantee,
  handleEditGuaranteePlan,
  handleModalEditGuarantee,
}: IModalEditGuarantee) => {
  ...
  return (
    <Modal title="Cadastrar garantia" setModal={handleModalEditGuarantee}>
      <Formik
        initialValues={{
          systemId: guarantee?.system?.id || '',
          description: guarantee?.description || '',
          observation: guarantee?.observation || '',
          failureTypesIds:
            guarantee?.guaranteeToFailureTypes?.map(({ failureType }) => failureType.id) || [],
          warrantyPeriod: (guarantee?.standardWarrantyPeriod || 12) / 12,
        ...
        onSubmit={async (values, actions) => {
          ...
          await handleEditGuaranteePlan({
            id: guarantee.id || '',
            systemId: values.systemId,
            description: values.description,
            observation: values.observation,
            failureTypesIds: values.failureTypesIds.filter((id) => id !== undefined) as string[],
            warrantyPeriod:
          ...
          setOnQuery(false);
          actions.resetForm();
          handleModalEditGuarantee(false);
        }}
      >
          ...
          <Style.FormContainer>
            ...
            {!loadingGuaranteeSystemsForSelect && !loadingGuaranteeFailureTypesForSelect && (
              <Form>
                ...
                <Button center label="Atualizar" type="submit" loading={onQuery} />
              </Form>
            )}
          </Style.FormContainer>
        )}
