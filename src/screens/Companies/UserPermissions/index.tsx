...
function UserPermissions() {
  ...
  const { buildings } = useBuildings({ companyId, filter: '', page: 1 });
  ...
  const handleGetAllPermissions = async () => {
    ...
    try {
      const responseData = await getAllPermissions({ adminPermissions: true });
    ...
    <>
      <Style.Header>
        <div>
          ...
          <ReturnButton path={`/companies/${companyId}`} />
        </div>
